...
smd(
  ...
  async (m) => {
    try {
      ...
      if (!query) {
        return await m.send("Please provide a song title, e.g., `.lyrics Naruto Blue Bird`.");
      }

      // Send a loading message
      await m.send("katakuri is writing the lyrics 🎵");
...
async(message, match) => {
 try{
   ...
   message.react("🔍")
         if (!match) return message.reply(`Give me a user name like ${prefix}github boru-to`)
...
smd({
    ...
    async(message, text) => {
 if(!text) return await message.reply('Give Me Number without + sign. Example: .iswa 2774781xxxx')
...
smd({
    ...
    async(message, text) => {
if(!text) return await message.reply('Give Me Number without + sign. Example: .nowa 2774781xxxx')
